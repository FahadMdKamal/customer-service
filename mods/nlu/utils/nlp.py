...
def req(logger, access_token, meth, path, params, **kwargs):
    ...
    if rsp.status_code > 200:
        raise NlpError('Wit responded with status: ' + str(rsp.status_code) +
                       ' (' + rsp.reason + ')')
    json = rsp.json()
    if 'error' in json:
        raise NlpError('Wit responded with an error: ' + json['error'])
...
class Nlp(object):
    ...
    def create_trait(self, trait_name, values, headers=None, verbose=None):
        ...
        print("data to show crate traits: {}".format(data))
        endpoint = '/traits'
        if verbose:
            params['verbose'] = verbose
        resp = req(self.logger, self.access_token, 'POST', endpoint, params, json=data, headers=headers)
