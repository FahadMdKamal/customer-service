...
# def extract_item(db_model:models, instance, item_list: list):
#     """
#     Recursively call and check if there is any child available.
#     """
#     if not isinstance(instance, type(None)):
#         db_objects = db_model.objects.filter(parent_id=instance)
#         if db_objects.exists():
#             for ct in db_objects:
#                 item_list.append(ct)
#                 extract_item(ct, item_list)
#             return item_list
#     return item_list


# def extract_raw_children(model_instance):
#     """
#     Returns Raw Children

#     Parameter: Model Instance
#     """
#     return extract_item(model_instance, [])


# def extract_serialized_children(model_instance, model_serializer):
#     """
#     Returns Serialized Children

#     Parameters: Model Instance, Model Serializer
#     """
#     children = [model_instance] + extract_raw_children(model_instance) 
...
class ExtractModelChildren:
    ...
    def __extract_item(self, instance, item_list: list):
        ...
        """
        if not isinstance(instance, type(None)):
            db_objects = self.model_class.objects.filter(parent__id=instance.id)
            if db_objects.exists():
                for ct in db_objects:
                    item_list.append(ct)
