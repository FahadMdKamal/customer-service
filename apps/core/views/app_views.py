...
class AppApiView(APIView):
    ...
    def get(self, request):
        if request.GET.get('app-code'):
            ...
            try:
                apps = Apps.objects.get(app_code=app_code.upper())
                serializer = AppSerializer(apps)
                return Response(serializer.data)    
            except ObjectDoesNotExist:
                return Response({"message":"No App Found"},status=status.HTTP_404_NOT_FOUND)
        else:
            obj_list = Apps.objects.all().order_by('-id')
            serializer = AppSerializer(obj_list, many=True)
...
class AppCreateOrUpdateApiView(APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.data.get('id'):
            db_object = Apps.objects.get(id=request.data.get('id'))
            serializer = AppSerializer(db_object, data=request.data, partial=True)
...
class AppDeleteApiView(APIView):
    ...
    def get(self, request):
        ...
        app = Apps.objects.filter(id=request.query_params.get('app-id'))
        if app:
            app.delete()
